...
public:
    ...
    // map -> odom
    tf::Transform map_to_odom; // 地图坐标系和imu里程计坐标系间的坐标变换
    ...
    gtsam::noiseModel::Diagonal::shared_ptr priorBiasNoise; // 先验的零偏噪声
    gtsam::noiseModel::Diagonal::shared_ptr correctionNoise; // 先验的位姿纠正噪声
    gtsam::Vector noiseModelBetweenBias; // 含零偏噪声模型


    gtsam::PreintegratedImuMeasurements *imuIntegratorOpt_; // 用于优化零偏的预积分器
    gtsam::PreintegratedImuMeasurements *imuIntegratorImu_; // 用于预测位姿的预积分器

    std::deque<sensor_msgs::Imu> imuQueOpt; // 用于优化零偏的imu数据队列
    std::deque<sensor_msgs::Imu> imuQueImu; // 用于预积分的imu数据队列

    gtsam::Pose3 prevPose_; // 优化的位姿
    gtsam::Vector3 prevVel_; // 优化的速度
    gtsam::NavState prevState_; // 优化的状态
    gtsam::imuBias::ConstantBias prevBias_; // 优化的零偏

    gtsam::NavState prevStateOdom; // 存放优化的状态，给预积分作初值
    gtsam::imuBias::ConstantBias prevBiasOdom; // 存放优化的零偏，给预积分作初值

    bool doneFirstOpt = false; // 是否完成第一次优化的标志位
    double lastImuT_imu = -1; // 上一个imu帧时间戳（预测队列）
    double lastImuT_opt = -1; // 上一个imu帧时间戳（优化队列）

    gtsam::ISAM2 optimizer; // gtsam优化器
    gtsam::NonlinearFactorGraph graphFactors; // gtsam因子图
    gtsam::Values graphValues; // 存放gtsam优化变量

    const double delta_t = 0; // 与当前帧时间间隔超过此变量的为旧imu帧

    int key = 1; // 当前因子索引，同时代表了因子图规模
    int imuPreintegrationResetId = 0; // 预积分ID，用来保证odom位姿与imu预积分位姿对齐

    gtsam::Pose3 imu2Lidar = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(-extTrans.x(), -extTrans.y(), -extTrans.z())); // imu到雷达平移外参
    gtsam::Pose3 lidar2Imu = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(extTrans.x(), extTrans.y(), extTrans.z()));; // 雷达到imu平移外参
    ...
    {
        double currentCorrectionTime = ROS_TIME(odomMsg); // 当前odom帧时间戳

        // make sure we have imu data to integrate 保证存在IMU帧信息用来预积分估计位姿
        if (imuQueOpt.empty())
            return;
        ...
        // correction pose jumped, reset imu pre-integration id不同步时，重置预积分系统
        ...
        if (systemInitialized == false)
        {
            ...
            // pop old IMU message 删除旧的IMU帧（odom时间戳以前的IMU帧）
            ...
            // initial pose 用odom得到的位姿初始化imu位姿，作为因子图第一个因子
            prevPose_ = lidarPose.compose(lidar2Imu);
            gtsam::PriorFactor<gtsam::Pose3> priorPose(X(0), prevPose_, priorPoseNoise); //X(0)表示第一个位姿
            graphFactors.add(priorPose);
            // initial velocity 初始化速度因子
            prevVel_ = gtsam::Vector3(0, 0, 0);
            gtsam::PriorFactor<gtsam::Vector3> priorVel(V(0), prevVel_, priorVelNoise); //V(0)表示第一个速度。这里先验速度为0，但是实际未必如此，所以这里的noise很大
            graphFactors.add(priorVel);
            // initial bias 初始化零偏因子
            prevBias_ = gtsam::imuBias::ConstantBias();
            gtsam::PriorFactor<gtsam::imuBias::ConstantBias> priorBias(B(0), prevBias_, priorBiasNoise); //B(0)表示第一个bias
        ...
        // reset graph for speed 当索引值达到100时，需要控制因子图规模
        if (key == 100)
        {
        ...
        // 1. integrate imu data and optimize 预积分旧的imu测量（这里的预积分用来优化零偏），并在队列中删除
        while (!imuQueOpt.empty())
        {
        ...
        graphFactors.resize(0);
        graphValues.clear();
        // Overwrite the beginning of the preintegration for the next step.  提取优化结果
        gtsam::Values result = optimizer.calculateEstimate();
        prevPose_  = result.at<gtsam::Pose3>(X(key));
        prevVel_   = result.at<gtsam::Vector3>(V(key));
        prevState_ = gtsam::NavState(prevPose_, prevVel_);
        prevBias_  = result.at<gtsam::imuBias::ConstantBias>(B(key));
        // Reset the optimization preintegration object.  更新预积分器的零偏
        imuIntegratorOpt_->resetIntegrationAndSetBias(prevBias_);
        ...
        // first pop imu message older than current correction data  删除激光里程计之前的imu数据
        double lastImuQT = -1;
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imuMsg)
    {
        ...
        // transform imu pose to ldiar  把odom坐标变换到lidar坐标系下（平移）
