...
class IntegrationBase
{
  public:
    ...
    double dt; // IMU测量时间间隔
    Eigen::Vector3d acc_0, gyr_0; // acc：前一帧的加速度测量  gyr：前一帧的角速度测量
    Eigen::Vector3d acc_1, gyr_1; // acc：当前帧的加速度测量  gyr：当前帧的角速度测量

    const Eigen::Vector3d linearized_acc, linearized_gyr; // 线性化的加速度和角速度（最初加速度、角速度）
    Eigen::Vector3d linearized_ba, linearized_bg; // 线性化的加速度角速度bias

    Eigen::Matrix<double, 15, 15> jacobian; // IMU雅可比，用于重新传播预积分
    Eigen::Matrix<double, 15, 15> covariance; // IMU协方差
    Eigen::Matrix<double, 18, 18> noise; // IMU噪声，用于计算IMU协方差

    double sum_dt; // 两图片帧之间的总时间间隔
    Eigen::Vector3d delta_p; // position增量
    Eigen::Quaterniond delta_q; // 四元数增量
    Eigen::Vector3d delta_v; // 速度增量

    // saves all the IMU measurements and time difference between two image frames
    std::vector<double> dt_buf; // 时间间隔buffer
    std::vector<Eigen::Vector3d> acc_buf; // 加速度buffer
    std::vector<Eigen::Vector3d> gyr_buf; // 角速度buffer
    ...
    // after optimization, repropagate pre-integration using the updated bias   采用最新的bias，恢复其他参数为初值，重新传播（预积分）
