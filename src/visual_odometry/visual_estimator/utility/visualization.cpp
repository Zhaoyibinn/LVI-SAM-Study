...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int& failureId)
{
    ...
    // Quternion not normalized  判断四元数是否归一化
    if (Q.x() * Q.x() + Q.y() * Q.y() + Q.z() * Q.z() + Q.w() * Q.w() < 0.99)
        return;

    // imu odometry in camera frame  相机系下IMU里程计
    ...
    // imu odometry in ROS format (change rotation), used for lidar odometry initial guess  ROS系下的里程计信息，给雷达里程计NDT匹配的赋初值
    odometry.pose.covariance[0] = double(failureId); // notify lidar odometry failure  通知雷达里程计_vins失败与否

    tf::Quaternion q_odom_cam(Q.x(), Q.y(), Q.z(), Q.w());
    // tf::Quaternion q_cam_to_lidar(0, 1, 0, 0); // mark: camera - lidar  相机到雷达系，Y轴旋转180°
    ...
    // ROS系下的VIO里程计信息，发布为"odom"与 "vins_world"坐标变换，为feature_tracker_node的特征点深度获取做准备
