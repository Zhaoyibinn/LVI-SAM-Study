...
void LoopDetector::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    // check loop if valid using ransan and pnp 使用ransan和pnp检查回环
	if (loop_index != -1)
	{
        // 获取回环候选帧
...
int LoopDetector::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    //first query; then add this frame into database! 查询字典数据库，得到与每一帧的相似度评分ret，并添加到字典
    QueryResults ret;
    ...
    // a good match with its nerghbour 通过相似度评分判断是否存在回环候选帧
    ...
    // 判断当前帧的索引值是否大于50，即系统开始的前50帧不进行回环；
