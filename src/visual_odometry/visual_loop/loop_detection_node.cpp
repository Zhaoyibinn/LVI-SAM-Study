...
Eigen::Vector3d tic; // 外参平移
Eigen::Matrix3d qic; // 外参旋转
...
void process()
{
    ...
    while (ros::ok())
    {
        ...
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())
        {
            ...
            // 排除以上两种情况，即为：最旧图片时间戳晚于最旧位姿时间戳  且  最旧图片时间戳晚于最旧特征点时间戳
            // 并加上两个条件：最新图片时间戳新于最旧位姿时间戳  且  最新特征点时间戳 新于 最旧位姿时间戳
            // 即 最旧图片时间戳位姿 <= 最旧位姿时间戳 <= 最新图片时间戳位姿
            //                             且  最旧位姿时间戳 <= 最新特征点时间戳 最旧图片时间戳 <= 最旧特征点时间戳
                ...
                && point_buf.back()->header.stamp.toSec() >= pose_buf.front()->header.stamp.toSec())
            {
                ...
                while (!pose_buf.empty())
                    pose_buf.pop();
                // 去掉旧于位姿数据的图片帧，提取最接近的图片帧
                while (image_buf.front()->header.stamp.toSec() < pose_msg->header.stamp.toSec())
                    image_buf.pop();
                image_msg = image_buf.front();
                image_buf.pop();

                // 去掉旧于位姿数据的特征点帧，提取最接近特征点数据
        ...
        if (pose_msg != NULL)
        {
            ...
            // add keyframe  如果平移距离足够大才认定为回环检测模块的关键帧，以控制频率
            if((T - last_t).norm() > SKIP_DIST)
            {
                ...
                // detect loop  添加新的关键帧并做回环检测。回环检测的主要函数
                m_process.lock();
...
int main(int argc, char **argv)
{
    ...
    if (LOOP_CLOSURE)
    {
        ...
        // initialize brief extractor 读取描述子pattern路径并载入pattern
        ...
        // initialize camera model 载入相机参数
    ...
    pub_match_img = n.advertise<sensor_msgs::Image>             (PROJECT_NAME + "/vins/loop/match_image", 3); // 发布回环帧图片和位姿用于可视化
    pub_match_msg = n.advertise<std_msgs::Float64MultiArray>    (PROJECT_NAME + "/vins/loop/match_frame", 3); // 发布视觉回环帧match_frame
