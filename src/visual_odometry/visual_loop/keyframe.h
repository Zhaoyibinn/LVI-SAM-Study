...
public:
	...
	// Pose in vins_world
	Eigen::Vector3d origin_vio_T; // 关键帧初始平移向量
	Eigen::Matrix3d origin_vio_R; // 关键帧初始旋转矩阵

	cv::Mat image; // 拷贝关键帧图像
	cv::Mat thumbnail; // 关键帧图像缩略图
	...
	bool searchInAera(const BRIEF::bitset window_descriptor,
	                  ...
	                  cv::Point2f &best_match_norm); // 计算某个原始特征点描述子与另一帧所有描述子的汉明距离，最小汉明距离足够小则返回true并保存匹配点
	...
	void searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_old,
                          ...
                          const std::vector<BRIEF::bitset> &descriptors_old,
                          const std::vector<cv::KeyPoint> &keypoints_old,
                          const std::vector<cv::KeyPoint> &keypoints_old_norm); // 对每个原始特征点与另一帧所有描述子执行searchInAera，记录结果并保存最小汉明的两个点坐标


	void PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
	               const std::vector<cv::Point3f> &matched_3d,
	               std::vector<uchar> &status); // RANSAC求解两帧PnP后的内点数量
};