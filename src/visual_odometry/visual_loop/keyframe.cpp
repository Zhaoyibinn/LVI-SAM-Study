...
// 初始化：时间戳，索引值，图，缩略图，特征点三维坐标数组，归一化坐标数组，像素坐标数组，特征点索引数组。提取描述子
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
    ...
    // 将此关键帧对象与某个回环帧进行BRIEF描述子匹配，并删除匹配失败的点
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
        ...
        if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
        {
        	if (pub_match_img.getNumSubscribers() != 0)
            {
                ...
                // 将image、gap_image、old_img水平拼接起来成为gray_img
                cv::hconcat(thumbnail, gap_image, gap_image);
                cv::hconcat(gap_image, old_img, gray_img);
                // 灰度图gray_img转换成RGB图loop_match_img
                cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);
                // 在图片loop_match_img上标注出匹配点和之间的连线
                // plot features in current frame 当前帧中画特征
                for(int i = 0; i< (int)matched_2d_cur.size(); i++)
                {
                    cv::Point2f cur_pt = matched_2d_cur[i] * MATCH_IMAGE_SCALE;
                    cv::circle(loop_match_img, cur_pt, 5*MATCH_IMAGE_SCALE, cv::Scalar(0, 255, 0)); //（图像，点中心，点半径，点颜色）
                }
                // plot features in previous frame 历史回环帧中画特征
                ...
                // plot lines connecting features 画特征间连线
                for (int i = 0; i< (int)matched_2d_cur.size(); i++)
                {
                    cv::Point2f old_pt = matched_2d_old[i] * MATCH_IMAGE_SCALE;
                    old_pt.x += thumbnail.size().width + gap;
                    cv::line(loop_match_img, matched_2d_cur[i] * MATCH_IMAGE_SCALE, old_pt, cv::Scalar(0, 255, 0), 2*MATCH_IMAGE_SCALE, 8, 0); //（图像，起点，终点，点颜色，线宽，线类型）
                }
                // plot text
                ...
                // publish matched image 发布 matched image
