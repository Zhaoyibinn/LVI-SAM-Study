...
class FeatureTracker
{
  public:
    ...
    cv::Mat mask; // 设置掩膜，使关键点均匀化
    cv::Mat fisheye_mask;
    cv::Mat prev_img, cur_img, forw_img;
    vector<cv::Point2f> n_pts; // 新提取到的关键点（未均匀化）
    vector<cv::Point2f> prev_pts, cur_pts, forw_pts; // 上一帧和当前帧像素坐标
    vector<cv::Point2f> prev_un_pts, cur_un_pts; //上一帧和当前值特征点去畸变相机归一化坐标
    vector<cv::Point2f> pts_velocity; //像素点x和y方向运动速度
    vector<int> ids; //特征点id索引 vector集合
    vector<int> track_cnt; //对应特征点跟踪次数
    map<int, cv::Point2f> cur_un_pts_map; //上一帧图像特征点map,id->坐标的map
    map<int, cv::Point2f> prev_un_pts_map; //上一帧图像特征点map,id->坐标的map
    camodocal::CameraPtr m_camera; // 相机类指针
    double cur_time; // 当前帧时间戳
    double prev_time; // 前一帧时间戳

    static int n_id; // 关键点索引
};
...
public:
    ...
    tf::TransformListener listener; // tf广播监听器
    tf::StampedTransform transform; // 带时间戳的tf数据

    const int num_bins = 360; // 模拟深度图的长和宽
    vector<vector<PointType>> pointsArray; // 搜索到的点云点构成的矩阵
    ...
    {
        // 0.1 initialize depth for return  初始化存储特征点深度的容器，大小为特征点数量，初值都为-1；
        sensor_msgs::ChannelFloat32 depth_of_point;
        depth_of_point.name = "depth";
        depth_of_point.values.resize(features_2d.size(), -1);

        // 0.2  check if depthCloud available 确认雷达点云是否可用，若为空直接返回；
        if (depthCloud->size() == 0)
            return depth_of_point;

        // 0.3 look up transform at current image time  查询当前图像帧时刻的变换，监听从世界坐标系到body坐标系的变换
        try{
        ...
        // 0.4 transform cloud from global frame to camera frame  将点云坐标系从世界坐标系变换到相机坐标系
        ...
        // 0.5 project undistorted normalized (z) 2d features onto a unit sphere  投影去畸变归一化的2d特征点到单位球面；
        ...
        // 3. project depth cloud on a range image, filter points satcked in the same region  投影深度图至离散化表格range image，实现同一区域点云滤波的效果；
        ...
        // 4. extract downsampled depth cloud from range image  从离散图range image中距离有效的点，形成新的深度点云图；
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        ...
        // 5. project depth cloud onto a unit sphere  将深度点云图投影至单位球面;
        ...
        // 6. create a kd-tree using the spherical depth cloud  用单位球面点云图构建kd树
        pcl::KdTreeFLANN<PointType>::Ptr kdtree(new pcl::KdTreeFLANN<PointType>());
        kdtree->setInputCloud(depth_cloud_unit_sphere);

        // 7. find the feature depth using kd-tree  使用KD树寻找距离图像特征最近的三个点,并求解相机中心经图像特征的向量到这三个点平面的距离；
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            {   
                //intensity代表range，A、B、C为三个最近的点的坐标
        ...
        // update depth value for return 仅返回深度超过3m
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
        ...
        // visualization project points on image for visualization (it's slow!)  根据相机模型，把激光雷达的3D点转换为2D,在图像可视化
